...
class ReportController extends Controller {
    ...
    public function daily_report(Request $request) {
        if ($request->isMethod('get')) {
            return view('backend.reports.daily_report');
        }
        else if ($request->isMethod('post')) {

            $date = Carbon::parse($request->date)->format('Y-m-d');

            $totalRepayment = LoanPayment::whereDate('paid_at', $date)
            ->sum('total_amount');

            $totalLoanGiven = Loan::whereDate('approved_date', $date)
            ->sum('applied_amount');

            $otherExpences = Expense::with('expense_category')->whereDate('expense_date', $date)
            ->get(['amount', 'expense_category_id']);

            $branches = Branch::with(['branchAccounts.branchAccountTransactions' => function ($query) use ($date) {
            ...
            return view('backend.reports.daily_report', compact('date', 'branches', 'totalRepayment', 'totalLoanGiven', 'otherExpences'));
